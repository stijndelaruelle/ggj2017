Shader "Custom/BattleTransitions"{
	Properties{
	...
	SubShader{
		...
		Pass{
			...
			fixed4 simplefragopen(v2f i) : SV_Target{
				if (0.5 - abs(i.uv.y - 0.5) < abs(_Cutoff) * 0.5) return _Color;
				return tex2D(_MainTex, i.uv);
			}
			...
			fixed4 frag(v2f i) : SV_Target{
				...
				if (_Distort) direction = normalize(float2((transit.r - 0.5) * 2, (transit.g - 0.5) * 2));

				fixed4 col = tex2D(_MainTex, i.uv + _Cutoff * direction);

				if (transit.b < _Cutoff) return col = lerp(col, _Color, _Fade);
				return col;
			}
			ENDCG
		}
